...
export const Swimlane = props => (
  <div className={ 'swimlane__container' }>
    { props.lanes.map((lane, idx) => (<Lane key={ idx } title={ lane.title } start={ props.start } end={ props.end } cards={ lane.cards } />)) }
  </div>
)
...
export const LaneDetail = props => (
  <div className={ 'swimlane__detail' }>
    { props.cards.map((card, idx) => (<Card key={ idx } laneStart={ props.start } laneEnd={ props.end } className={ card.className } style={ card.style } description={ card.description } start={ card.start } end={ card.end } />)) }
  </div>
)
